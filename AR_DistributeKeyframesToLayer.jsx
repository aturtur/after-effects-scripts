/*
AR_DistributeKeyframesToLayer

Author: Arttu Rautio (aturtur)
Website: http://aturtur.com/
Name-US: AR_DistributeKeyframesToLayer
Description-US: Distributes selected keys evenly. Supports multiple selections, but does not support multiple selections on same property.
...
function newKeyframes(array) {
    for (var i = 0; i < array.length; i++) { // Loop through array
        ...
        for (var s = array[i]["FirstKey"]; s <= array[i]["LastKey"]; s++){ // Loop through keyframes
            var layerIn = array[i]["layer"].inPoint; // Get layer's in point
            var layerOut = array[i]["layer"].outPoint; // Get layer's out point
            var method = layerIn + (layerOut - layerIn) / (array[i]["KeyCount"] - 1) * (s-1); // Distribute keyframes to layer (core algorithm)
...
app.beginUndoGroup("AR_DistributeKeyframesToLayer"); // Begin undo group
