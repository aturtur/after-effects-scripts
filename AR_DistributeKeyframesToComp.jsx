/*
AR_DistributeKeyframesToComp

Author: Arttu Rautio (aturtur)
Website: http://aturtur.com/
Name-US: AR_DistributeKeyframesToComp
Description-US: Distributes selected keys to composition. Supports multiple selections, but does not support multiple selections on same property.
...
function newKeyframes(array) {
    ...
    for (var i = 0; i < array.length; i++) { // Loop through array
        ...
        for (var s = array[i]["FirstKey"]; s <= array[i]["LastKey"]; s++){ // Loop through keyframes
            var method = comp.duration / (array[i]["KeyCount"] - 1) * (t); // Distribute keyframes to composition (core algorithm)
...
app.beginUndoGroup("AR_DistributeKeyframesToComp"); // Begin undo group
