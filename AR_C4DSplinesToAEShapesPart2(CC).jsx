/*
AR_C4DSplinesToAEShapesPart2

Author: Arttu Rautio (aturtur)
Website: http://aturtur.com/
Name-US: AR_C4DSplinesToAEShapesPart2
Description-US: Imports AI sequence folder and creates animated shape(s).
Written for Adobe After Effects CC 2019 (Version 16.0.1 Build 48)
*/
//@target aftereffects
app.beginUndoGroup("c4d-splines-to-ae-shapes"); // Begin undo group
// -------------------------------------------------------------------------------------
...
for (var x=1; x<=comp.layers.length; x++){
    ...
    for (var g=1; g<=comp.layers[x].property("ADBE Root Vectors Group").numProperties; g++){currPaths++;}
...
var myLayer=comp.layers.addShape();

for (var n=0; n<numPaths; n++){
    //var myNewShapeGroup = myLayer.property("Contents").property("ADBE Root Vectors Group").property("ADBE Vector Shape - Group");
    
    var shapeGroup = myLayer.property("Contents").addProperty("ADBE Vector Group");
    var myNewShapePath = shapeGroup.property("Contents").addProperty("ADBE Vector Shape - Group");
        
}
for (var i=2; i<=comp.layers.length; i++){
    for (var gp=1; gp<=comp.layers[i].property("ADBE Root Vectors Group").numProperties; gp++){

        // SHAPE DATA
        // FROM
        var myShape=comp.layers[i].property("ADBE Root Vectors Group").property(gp).property("ADBE Vectors Group").property("ADBE Vector Shape - Group").property("ADBE Vector Shape").valueAtTime(comp.layers[i].inPoint, false);
        // TO
        comp.layers[1].property("ADBE Root Vectors Group").property(gp).property("ADBE Vectors Group").property("ADBE Vector Shape - Group").property("ADBE Vector Shape").setValueAtTime(comp.layers[i].inPoint, myShape);
        
        // TRANSFORMATION (GENERAL)
        // FROM 
        var anchorPoint = comp.layers[i].property("ADBE Transform Group").property("ADBE Anchor Point").valueAtTime(comp.layers[i].inPoint, false);
        var position = comp.layers[i].property("ADBE Transform Group").property("ADBE Position").valueAtTime(comp.layers[i].inPoint, false);
        var rotation = comp.layers[i].property("ADBE Transform Group").property("ADBE Rotate Z").valueAtTime(comp.layers[i].inPoint, false);
        // TO
        comp.layers[1].property("ADBE Transform Group").property("ADBE Anchor Point").setValueAtTime(comp.layers[i].inPoint, anchorPoint);
        comp.layers[1].property("ADBE Transform Group").property("ADBE Position").setValueAtTime(comp.layers[i].inPoint, position);
        comp.layers[1].property("ADBE Transform Group").property("ADBE Rotate Z").setValueAtTime(comp.layers[i].inPoint, rotation);
        
        // TRANSFORMATION (VECTOR GROUP)
        var vectorAnchorPoint =comp.layers[i].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Anchor").valueAtTime(comp.layers[i].inPoint, false);
        var vectorPosition =comp.layers[i].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Position").valueAtTime(comp.layers[i].inPoint, false);
        var vectorRotation =comp.layers[i].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Rotation").valueAtTime(comp.layers[i].inPoint, false);
        
        // TO
        comp.layers[1].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Anchor").setValueAtTime(comp.layers[i].inPoint, vectorAnchorPoint);
        comp.layers[1].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Position").setValueAtTime(comp.layers[i].inPoint, vectorPosition);
        comp.layers[1].property("ADBE Root Vectors Group").property(gp).property("ADBE Vector Transform Group").property("ADBE Vector Rotation").setValueAtTime(comp.layers[i].inPoint, vectorRotation);

    }
    // Write current progress to info panel
    percent = Math.round((i/comp.layers.length)*100); // Calculate progress
    clearOutput(); // Clear old stuff from info panel
    writeLn("Animating shapes..."); // Write stuff to info panel
    writeLn("Progress: "+percent.toString()+ " %"); // Write stuff to info panel
}
