/*
AR_DistributeKeyframesByStep

Author: Arttu Rautio (aturtur)
Website: http://aturtur.com/
Name-US: AR_DistributeKeyframesByStep
Description-US: Distributes selected keys with given step. Supports multiple selections, but does not support multiple selections on same property.
...
function newKeyframes(array) {
    ...
    var interval = prompt("Step in frames"); // Input prompt to get step size
    ...
    for (var i = 0; i < array.length; i++) { // Loop through array
        ...
        for (var s = array[i]["FirstKey"]; s <= array[i]["LastKey"]; s++){ // Loop through keyframes
            var method = array[i]["FirstTime"] + (interval/frameRate) * (t); // Distribute keyframes by step (core algorithm)
...
app.beginUndoGroup("AR_DistributeKeyframesByStep"); // Begin undo group
